...
export class RaceWriterService {
    ...
    private writeSessionTiming(session: Session): string {
        ...
        session.laps.forEach((lap: Lap, index: number) => {
            ...
            let eventTimes = lap.sectors
                // Splits for the sectors
                .map(sector => sector.split)
                // Current lap penalties
                .concat(...lap.penalties.map(penalty => penalty.lapTime))
                // Previous lap penalties (accounting for times beyond the current lap)
                .concat(...lap.previousBest.penalties.map(penalty => penalty.lapTime < lap.lapTime ? penalty.lapTime : lap.lapTime));
    ...
    private getTimingRow(sessionTime: number, lapTime: number, race: Race, session: Session, currentLap: Lap, previousLap: Lap): Object {
        ...
        // Previous Penalties (account for times beyond the end of the current lap)
        timingData["Previous Penalty"] = lapTime < currentLap.lapTime ? previousLap.getPenaltyCount(lapTime) : previousLap.getPenaltyCount(previousLap.lapTime);
